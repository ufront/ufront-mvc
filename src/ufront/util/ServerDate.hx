...
/** 
	On the server, provide getJsFragment(), which gives you a piece of JS which will tell the current server time to the client, so the offset can be calculated.
	
	On the client, provide a ServerDate.now() function which behaves similarly to Date.now(), but with the time being offset so that it is approximately the same as the server.
	
	This does nothing to account for HTTP latency etc, it will merely make sure that you're about the same, as in, not minutes or hours or years in the wrong date.
**/
class ServerDate
{
	#if server 
		/**
			...
			Insert this anywhere in your HTML, and by the time your DOM is ready, the client can use 

			    `ServerDate.now()` 
			
			and the offset will be calculated correctly
		**/
		...
		**/
		...
		/** 
			...
			This is based on a "window.__ufront_serverTimeAtLoad" property being present in Javascript.  
			It is compared to "clieantTimeAtLoad" to figure out the offset.  
			If "window.__ufront_serverTimeAtLoad" is undefined, we'll just use the clientTimeAtLoad, and so the offset will be 0, and this will be the same as running Date.now()
		**/
